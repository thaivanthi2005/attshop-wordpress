...
<section id="post-section" class="post-section theme-default">
	<div class="container-full" style="display: flex;">
		<?php
			$neom_dark_archive_blog_pages_layout = get_theme_mod( 'neom_archive_blog_pages_layout', 'neom_right_sidebar' );
			if ( 'neom_left_sidebar' === $neom_dark_archive_blog_pages_layout ) :
				get_sidebar();
					endif;
			?>
		<div id="awp-main-content" class="<?php echo esc_attr( neom_post_layout() ); ?>">
			...
			<div class="theme-columns-area">
				...
				<?php
					$neom_dark_paged = ( get_query_var( 'paged' ) ) ? get_query_var( 'paged' ) : 1;
					$args       = array(
						'post_type' => 'post',
						'paged'     => $neom_dark_paged,
					);
					?>
				<?php if ( have_posts() ) : ?>
					<?php
		...
		<?php if ( 'neom_right_sidebar' === $neom_dark_archive_blog_pages_layout ) : ?>
				<?php get_sidebar(); ?>
			<?php endif; ?>
	</div>
</section>
